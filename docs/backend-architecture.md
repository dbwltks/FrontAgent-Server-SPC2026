...
- 웹 채팅과 브라우저 WebRTC 기반 웹 음성 통화 처리
...
- 단순 질문은 원문 검색어 1개를 사용하고, 복합 질문만 하위 질문으로 분리합니다.
- 원문 질문을 항상 포함하며 중복 제거 후 최대 3개로 제한합니다.
- 질문별 검색을 `asyncio.gather()`로 병렬 실행합니다.
- 질문별 검색은 3초 timeout을 적용합니다.
- 각 질문당 최대 3개 chunk를 사용합니다.
- 질문별 결과를 합친 뒤 similarity 순으로 정렬하고 전체 6개 chunk로 제한합니다.
...
sequenceDiagram
    ...
    participant V as POST /voice/realtime
    participant O as OpenAI Realtime
    participant C as POST /chat
    participant G as LangGraph

    B->>V: WebRTC SDP offer
    V->>O: SDP + Realtime session 설정
    O-->>V: SDP answer
    V-->>B: SDP answer
    B->>O: 사용자 음성
    O-->>B: query_agent 함수 호출
    B->>C: 인식 문장, channel=web_call
    C->>G: 기존 상담 그래프 실행
    G-->>B: 최종 텍스트 답변
    B->>O: function_call_output
    O-->>B: 답변 음성
```

Realtime 모델은 직접 업무 답변을 만들지 않고 매 발화마다 `query_agent`를 호출합니다. 따라서 기존 LangGraph의 규칙, 지식 검색, 멀티턴 상태와 실행 로그가 웹 통화에도 동일하게 적용됩니다.
...
웹 통화의 오디오는 WebRTC로 송수신하고, Realtime 함수 호출은 WebRTC data channel로 처리합니다.
